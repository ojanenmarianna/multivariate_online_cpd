...
# https://github.com/lightonai/newma/blob/master/onlinecp/algos.py
"""
...
Implements NEWMA and Scan-B algorithms.
"""
...
class OnlineCP(ABC):
    ...
    store_values: bool
        ...
        if False, does not store anything, and self.update(sample) returns the detection
        result directly
    ...
    def __init__(self,
        ...
        # NOTE: the adaptive threshold is based on the assumption that the squared statistic
    ...
    def flag_sample(self):
        ...
        if self.thresholding_method == 'adapt':
            return self.statistic > np.sqrt(
                self.adapt_mean + self.thresholding_mult *\
                    np.sqrt(self.adapt_second_moment - self.adapt_mean ** 2))
    ...
    def update(self, new_sample):
        ...
        self.adapt_mean = (
            1 - self.adapt_forget_factor) * self.adapt_mean + self.adapt_forget_factor * \
                self.statistic ** 2
        self.adapt_second_moment = (
            1 - self.adapt_forget_factor) * self.adapt_second_moment + \
                self.adapt_forget_factor * self.statistic ** 4
...
class NEWMA(OnlineCP):
    ...
    init_sample: NP.NDARRAY (d,),
        bogus sample, ewma stats are initialized with feat_func(init_sample).
        Most often np.zeros(d).
    forget_factor: float,
...
class ScanB(OnlineCP):
    ...
    Remark: we use the BIASED estimator of the MMD (ie with diagonal terms), because the unbiased
    ...
    kernel_sum_XY: kernel of samples in time window X wrt samples in time window possibly
    post change
    ...
    def update_kernel_sum(self, val, datax, datay, newx, newy, oldx, oldy):
        return val + (self.kernel_func(datax, newy).sum() - self.kernel_func(datax, oldy).sum()
                      ...
                      + self.kernel_func(newx[np.newaxis, :], newy) - \
                          self.kernel_func(oldx[np.newaxis, :], oldy)) / (
    ...
    def update_stat(self, sample):
        ...
        return self.kernel_sum_XX.sum() / self.N + self.kernel_sum_YY - 2 * \
            self.kernel_sum_XY.sum() / self.N

#%% additional utils

def compute_adapt_threshold(stat, thresholding_quantile=0.95, adapt_forget_factor=0.05):
    """
    Compute adaptive threshold from a detection statistic, based on the assumption that its square
    is approx Gaussian
    stat: array (T,)
    """
